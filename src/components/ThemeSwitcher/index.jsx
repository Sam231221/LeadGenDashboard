...
export default function ThemeSwitcher() {
  ...
  return (
    <>
      ...
      >
        {mode === "light" && (
          <BiSun className=" pointer-events-none" size={20} />
        )}
        {mode === "dark" && (
          <MdOutlineDarkMode className="pointer-events-none" size={20} />
        )}
        {mode === "system" && (
          <MdComputer className="pointer-events-none" size={20} />
        )}
      </div>

      <ul
        className={`${
          showOptions === true ? "right-0 " : "-right-60"
              ...
              >
                <span
                  className={`w-3 h-3 rounded-full ${
                    color === item.id ? "bg-[var(--togglerColor)]" : "bg-white"
                  }`}
                ></span>
              </span>
